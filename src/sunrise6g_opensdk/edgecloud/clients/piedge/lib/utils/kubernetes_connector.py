...
class KubernetesConnector:
    ...
    def delete_service_function(self, connector_db: ConnectorDB, service_function_name):
        ...
        deleted_app = self.api_instance_appsv1.delete_namespaced_deployment(
            name=service_function_name, namespace="sunrise6g"
        )

        deleted_service = self.v1.delete_namespaced_service(
            name=service_function_name, namespace="sunrise6g"
        )
        ...
        for hpa in hpa_list.items:
            if hpa.metadata.name == service_function_name:
                deleted_hpa = self.api_instance_v1autoscale.delete_namespaced_horizontal_pod_autoscaler(
                    name=service_function_name, namespace="sunrise6g"
                )
                break
        # deletevolume
    ...
    def deploy_service_function(self, descriptor_service_function):
        ...
        try:
            ...
            api_response_service = self.v1.create_namespaced_service(
                ...
                for metrics in catalogue_policy["monitoring_metrics"]:
                    ...
                    if metrics["metric_name"] == hpa["deployed_metric"]:
                        ...
                        if (
                            ...
                            api_instance_v1autoscale.patch_namespaced_horizontal_pod_autoscaler(
                                namespace="sunrise6g",
                                name=desc_paas["name"],
                                body=body_hpa,
                            )
...
def get_deployed_hpas():
    ...
    api_response = api_instance_v1autoscale.list_namespaced_horizontal_pod_autoscaler(
        "sunrise6g"
    )
